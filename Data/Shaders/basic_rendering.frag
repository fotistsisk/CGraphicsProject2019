...
uniform sampler2D diffuse_texture; // travame to xrwma tou kathe antikeimenou
...
uniform sampler2D shadowmap_texture; // times vathous!!

float uniform_constant_bias = 0.0001; // peirazw to shmeio kai mperdeuw pou mpainei h skia
...
float shadow_pcf2x2(vec3 light_space_xyz)
{
	...
	vec2 uv = light_space_xyz.xy - 0.5 * vec2(xOffset, yOffset); //ta kanw ola 2D
	float u_ratio = mod(uv.x, xOffset) / xOffset; // poso apexw apo to kentro ston x'x
	float v_ratio = mod(uv.y, yOffset) / yOffset; //poso apexw apo to kentro ston y'y
...
float shadow_nearest(vec3 light_space_xyz)
{
	...
	// + shaded -> 0.0
	// - lit -> 1.0
	return (light_space_xyz.z - uniform_constant_bias < shadow_map_z) ? 1.0 : 0.0; // den mhdenizw thn sunisfora tou fwtismou an epistrefw 1
}
...
float shadow(vec3 pwcs)
{
	...
	plcs.xy = (plcs.xy + 1) * 0.5; // giati ta textures theloun times sto [0,1]
	...
	return shadow_pcf2x2(plcs.xyz); // eksomalinsi ths skias
}
...
void main(void)
{
	...
	vec4 diffuseColor = vec4(uniform_diffuse.rgb, 0.6);
	...
	out_color = vec4( diffuseReflection + specularReflection, diffuseColor.a);	//for blending
}
